...
public:
    ...
    static lw_shared_ptr<query::read_command> make_paged_read_cmd(const schema& s, uint32_t column_limit, const std::string* start_column, const partition_key& start_key) {
        ...
        if (s.thrift().is_dynamic()) {
            ...
            if (start_column) {
                auto sr = query::specific_ranges(start_key, {make_clustering_range(s, *start_column, std::string())});
    ...
    static future<> do_get_paged_slice(
        ...
        auto cmd = make_paged_read_cmd(*schema, column_limit, start_column, *range.start()->value().key());
        auto end = range.end();
        if (start_column && !schema->thrift().is_dynamic()) {
            ...
            range = query::partition_range(std::move(range.start()), std::move(start));
        }
        return service::get_local_storage_proxy().query(schema, cmd, {std::move(range)}, consistency_level).then([schema, cmd, column_limit](auto result) {
        ...
        }).then([schema, cmd, column_limit, consistency_level, end = std::move(end), start_column, &output](auto&& slices) {
            ...
            if (columns == 0 || columns == column_limit || (slices.size() < cmd->partition_limit && columns < cmd->row_limit)) {
                ...
                // Otherwise, the single partition we queried may have been empty, so retry with no start column.
            }
...
private:
    ...
    concept bool Aggregator() {
        return requires (T* aggregation, const bytes_view& name, const query::result_atomic_cell_view& cell) {
            typename type = T;
            { on_column(aggregation, name, cell) };
        };
    }
    */
