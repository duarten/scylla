        ...
        with_cob(std::move(cob), std::move(exn_cob), [&]{
            ...
            return db::get_local_size_estimates_recorder().record_size_estimates()
                ...
                return db::system_keyspace::query_size_estimates(current_keyspace(), std::move(cf), std::move(tstart), std::move(tend))
                    ...
                    auto emplace_acc = [&] {
                        splits.emplace_back();
                        splits.back().__set_start_token(bytes_to_string(to_bytes_view(acc.range_start_token)));
                        splits.back().__set_end_token(bytes_to_string(to_bytes_view(acc.range_end_token)));
