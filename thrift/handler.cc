...
public:
    ...
    static lw_shared_ptr<query::read_command> make_paged_read_cmd(const schema& s, uint32_t column_limit, const std::string* start_column) {
        ...
        if (is_dynamic(s)) {
            ...
            // any excess cells. Like with dynamic CFs, we issue a first query to consume remaining cells in a row.
            row_limit = query::max_rows;
            partition_limit = (column_limit + s.regular_columns_count() - 1) / s.regular_columns_count();
            schema::const_iterator start_col;
            if (start_column) {
                start_col = s.regular_lower_bound(to_bytes(*start_column));
                if (start_col != s.regular_begin()) {
                    partition_limit = 1;
                }
            } else {
                start_col = s.regular_begin();
            }
            clustering_ranges.emplace_back(query::clustering_range::make_open_ended_both_sides());
            add_columns(start_col, s.regular_end(), regular_columns, false);
        }
    ...
    static future<> do_get_paged_slice(
        ...
        auto cmd = make_paged_read_cmd(*schema, column_limit, start_column);
        auto end = range.end();
        return service::get_local_storage_proxy().query(schema, cmd, {std::move(range)}, consistency_level).then([schema, cmd, column_limit](auto result) {
        ...
        }).then([schema, cmd, column_limit, consistency_level, end = std::move(end), &output](auto&& slices) {
            ...
            auto start = dht::global_partitioner().decorate_key(*schema, key_from_thrift(*schema, to_bytes_view(slices.back().key)));
       ...
       with_cob(std::move(cob), std::move(exn_cob), [&] {
            ...
            if (is_dynamic(s)) {
                ...
                auto on_range = [&](auto&& range) {
                    ...
                    add_columns(start, end, regular_columns, request.reversed);
                };
                if (request.reversed) {
...
private:
    ...
    static partition_key key_from_thrift(const schema& s, bytes_view k) {
        ...
        return partition_key::from_exploded(composite.components() | boost::adaptors::transformed([](auto&& c) { return c.first; }));
    }
    ...
    static clustering_key_prefix make_clustering_prefix(const schema& s, bytes_view v) {
        if (s.thrift().has_compound_comparator()) {
            ...
            return clustering_key_prefix::from_exploded(composite.components() | boost::adaptors::transformed([](auto&& c) { return c.first; }));
        } else {
            assert(s.clustering_key_size() == 1);
            return clustering_key_prefix::from_single_value(s, to_bytes(v));
        }
    }
    static query::clustering_range::bound make_clustering_bound(const schema& s, bytes_view v) {
        ...
        if (s.thrift().has_compound_comparator()) {
            ...
            auto&& ck = clustering_key_prefix::from_exploded(composite.components() | boost::adaptors::transformed([&](auto&& c) {
                inclusive = c.second != composite::eoc::start;
                return c.first;
            }));
            return bound(std::move(ck), inclusive);
        } else {
            assert(s.clustering_key_size() == 1);
            return bound(clustering_key_prefix::from_single_value(s, to_bytes(v)));
        }
    }
    static query::clustering_range make_clustering_range(const schema& s, const std::string& start, const std::string& end) {
        ...
        if (!start.empty()) {
            start_bound = make_clustering_bound(s, to_bytes_view(start));
        }
        stdx::optional<bound> end_bound;
        if (!end.empty()) {
            end_bound = make_clustering_bound(s, to_bytes_view(end));
        }
    ...
    static void add_columns(auto&& beg, auto&& end, std::vector<column_id>& out, bool reversed) {
         while (beg != end) {
    ...
    static lw_shared_ptr<query::read_command> slice_pred_to_read_cmd(const schema& s, const SlicePredicate& predicate) {
        ...
        if (predicate.__isset.column_names) {
            ...
            if (is_dynamic(s)) {
                ...
                add_columns(r.first, r.second, regular_columns, range.reversed);
            }
        } else {
    ...
    static Column make_column(const bytes_view& col, const query::result_atomic_cell_view& cell) {
        Column ret;
    ...
    static ColumnOrSuperColumn make_column_or_supercolumn(const bytes_view& col, const query::result_atomic_cell_view& cell) {
    ...
    public:
        ...
        void accept_new_row(const clustering_key_prefix& key, const query::result_row_view& static_row, const query::result_row_view& row) {
            ...
            if (cell && _cell_limit > 0) {
                if (_s.thrift().has_compound_comparator()) {
                    bytes column_name = composite::serialize_value(key.components());
                    Aggregator::on_column(_current_aggregator, column_name, *cell);
                } else {
                    Aggregator::on_column(_current_aggregator, *key.begin(_s), *cell);
                }
                _cell_limit -= 1;
            }
        }
    ...
    struct column_or_supercolumn_builder {
        ...
        void on_column(std::vector<ColumnOrSuperColumn>* current_cols, const bytes_view& name, const query::result_atomic_cell_view& cell) {
    ...
    static range_tombstone make_range_tombstone(const schema& s, const SliceRange& range, tombstone tomb) {
        ...
        if (!range.start.empty()) {
            start_bound = make_clustering_bound(s, to_bytes_view(range.start));
        }
        stdx::optional<bound> end_bound;
        if (!range.finish.empty()) {
            end_bound = make_clustering_bound(s, to_bytes_view(range.finish));
        }
