...
public:
    ...
    virtual sstable_writer* select_sstable_writer(const dht::decorated_key& dk) override {
        if (!_writer) {
            ...
            _writer.emplace(_sst->get_writer(*_cf.schema(), partitions_per_sstable(), cfg, flush_permit::unconditional(), priority));
        }
        return &*_writer;
    }
...
public:
    ...
    sstable_writer* select_sstable_writer(const dht::decorated_key& dk) override {
        ...
        if (!writer) {
            ...
            cfg.max_sstable_size = _max_sstable_size;
            auto&& priority = service::get_local_compaction_priority();
            writer.emplace(sst->get_writer(*_cf.schema(), partitions_per_sstable(), cfg, flush_permit::unconditional(), priority, _shard));
        }
        return &*writer;
    }
