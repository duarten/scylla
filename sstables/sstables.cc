...
static auto write_range_tombstone_bound(file_writer& out, const schema& s, const composite& clustering_element, const std::vector<bytes_view>& column_names, composite::eoc marker = composite::eoc::none) {
    if (!service::get_local_storage_service().cluster_supports_reading_correctly_serialized_range_tombstones()) {
        return write_column_name(out, clustering_element, column_names, marker);
    }
    return write_column_name(out, s, clustering_element, column_names, marker);
}
