...
sstable_writer::sstable_writer(sstable& sst, const schema& s, uint64_t estimated_partitions,
                               const sstable_writer_config& cfg, sstable_write_permit&& permit, const io_priority_class& pc, shard_id shard)
    : _sst(sst)
    , _schema(s)
    , _pc(pc)
    , _backup(cfg.backup)
...
sstable_writer sstable::get_writer(const schema& s, uint64_t estimated_partitions, const sstable_writer_config& cfg, sstable_write_permit&& permit, const io_priority_class& pc, shard_id shard)
{
...
future<> sstable::write_components(
    ...
    return seastar::async(std::move(attr), [this, mr = std::move(mr), estimated_partitions, schema = std::move(schema), cfg, permit = std::move(permit), &pc] () mutable {
