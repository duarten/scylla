...
class key {
    ...
    composite _c;
public:
    key(bytes&& b) : _kind(kind::regular), _c(std::move(b)) {}
    ...
    static key from_deeply_exploded(const schema& s, const std::vector<data_value>& v) {
        ...
        if (pt.size() > 1) {
            return key(composite::serialize_value(v | boost::adaptors::sliced(0, pt.size() - 1)));
        }
        return key(composite(v[0].serialize(), false));
    }
    template<typename RangeOfSerializedComponents>
    static key from_exploded(const schema& s, std::vector<bytes>& v) {
        auto& pt = s.partition_key_type()->types();
        if (pt.size() > 1) {
            return key(composite::serialize_value(v));
        }
        return key(composite(v[0], false));
    }
    static key from_exploded(const schema& s, std::vector<bytes>&& v) {
        auto& pt = s.partition_key_type()->types();
        if (pt.size() > 1) {
            return key(composite::serialize_value(std::move(v)));
        }
        return key(composite(std::move(v[0]), false));
    }
    ...
    static key from_partition_key(const schema& s, partition_key_view pk) {
        auto& pt = s.partition_key_type()->types();
        if (pt.size() > 1) {
            return key(composite::serialize_value(pk));
        }
        return key(composite(to_bytes(*pk.begin(s)), false));
    }
