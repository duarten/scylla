...
future<> migration_manager::notify_create_view(const schema_ptr& view) {
...
future<> migration_manager::notify_update_view(const schema_ptr& view, bool columns_changed) {
...
future<> migration_manager::notify_drop_view(const schema_ptr& view) {
    return seastar::async([this, view] {
        auto&& ks_name = view->ks_name();
        auto&& view_name = view->cf_name();
        for (auto&& listener : _listeners) {
...
future<> migration_manager::announce_new_view(schema_ptr view, bool announce_locally)
{
#if 0
    ...
    try {
        ...
        throw exceptions::configuration_exception(sprint("Cannot add table '%s' to non existing keyspace '%s'.", view->cf_name(), view->ks_name()));
    }
}

future<> migration_manager::announce_view_update(schema_ptr view, bool announce_locally)
{
#if 0
    view.metadata.validate();
#endif
    ...
    try {
        ...
        auto mutations = db::schema_tables::make_update_view_mutations(std::move(keyspace), old_view, std::move(view), api::new_timestamp());
        return announce(std::move(mutations), announce_locally);
