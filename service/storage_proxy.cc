...
class data_read_resolver : public abstract_read_resolver {
    ...
    class per_partition_short_read {
        uint32_t _short_read_diff{};
        uint32_t _live_count_for_biggest_diff;
    public:
        void register_live_count(const std::vector<version>& replica_versions, uint32_t reconciled_live_rows, uint32_t limit) {
            ...
            if (max_replica_live >= limit && reconciled_live_rows < limit) {
                if (limit - reconciled_live_rows > _short_read_diff) {
                    _short_read_diff = limit - reconciled_live_rows;
                    _live_count_for_biggest_diff = reconciled_live_rows;
                }
            }
        }
        ...
        uint32_t live_count() const {
            return _live_count_for_biggest_diff;
        }
    };

    size_t _total_live_count = 0;
    uint32_t _max_live_count = 0;
    per_partition_short_read _per_partition;
...
private:
    ...
    static stdx::optional<row_address> get_last_row(const schema& s, bool is_reversed, uint32_t row_limit, std::vector<std::vector<version>> versions, uint32_t replica) {
        uint32_t row_count = 0;
        const partition* last_partition = nullptr;
        for (auto&& pv : versions) {
            auto partition_row_count = pv[replica].par.row_count();
            if (!last_partition && partition_row_count > 0) {
                last_partition = &pv[replica].par;
            }
            row_count += partition_row_count;
        }
        stdx::optional<row_address> ret;
        if (last_partition && row_count >= row_limit) {
...
public:
    ...
    bool any_partition_short_read() const {
        return _per_partition.short_read();
    }
    uint32_t partition_live_count() const {
        return _per_partition.live_count();
    }
    ...
    stdx::optional<reconcilable_result> resolve(schema_ptr schema, const query::read_command& cmd, uint32_t original_row_limit, uint32_t original_per_partition_limit) {
        ...
        boost::range::transform(boost::make_iterator_range(versions.begin(), versions.end()), std::back_inserter(reconciled_partitions),
            ...
            _per_partition.register_live_count(v, live_row_count, original_per_partition_limit);
        ...
        if (has_diff) {
            if (_total_live_count >= original_row_limit && !_per_partition.short_read()
        ...
        data_resolver->done().then_wrapped([this, exec, data_resolver, cmd = std::move(cmd), cl, timeout] (future<> f) {
            try {
                ...
                if (rr_opt && (data_resolver->max_live_count() < cmd->row_limit || rr_opt->row_count() >= original_row_limit())
                    ...
                    auto result = ::make_foreign(::make_lw_shared(to_data_query_result(std::move(*rr_opt), _schema, _cmd->slice, _cmd->partition_limit)));
                    ...
                    if (data_resolver->any_partition_short_read()) {
                        // The number of live rows may have been bound by the per partition limit.
                        auto new_limit = x(cmd->slice.partition_row_limit(), data_resolver->partition_live_count());
                        _retry_cmd->slice.set_partition_row_limit(new_limit);
                        _retry_cmd->row_limit = std::max(cmd->row_limit,
