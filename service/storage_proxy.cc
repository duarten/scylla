...
class data_read_resolver : public abstract_read_resolver {
    ...
    struct version {
        ...
        partition par;
        bool empty;
        version(gms::inet_address from_, partition par_, bool empty_ = false)
                : from(std::move(from_)), par(std::move(par_)), empty(empty_) {}
    };
    ...
    uint32_t _max_partition_live_count;
...
private:
    ...
    static stdx::optional<row_address> get_last_row(const schema& s, bool is_reversed, uint32_t row_limit, const std::vector<std::vector<version>>& versions, uint32_t replica) {
        ...
        for (auto&& pv : versions) {
            auto& v = pv[replica];
            if (!last_partition && !v.empty) {
                last_partition = &v.par;
            }
            row_count += v.par.row_count();
        }
        ...
        do {
            ...
            for (reply& r : _data_results) {
                ...
                if (pit != r.result->partitions().rend() && pit->mut().key(s).legacy_equal(s, max_key)) {
                    ...
                    v.emplace_back(r.from, partition(0, freeze(mutation(max_key, schema))), true);
                }
            }
        } while(true);
        ...
        data_resolver->done().then_wrapped([this, exec, data_resolver, cmd = std::move(cmd), cl, timeout] (future<> f) {
            try {
                ...
                if (rr_opt && (data_resolver->max_live_count() < cmd->row_limit || rr_opt->row_count() >= original_row_limit())
                    ...
                    if (data_resolver->any_partition_short_read() || (!rr_opt && data_resolver->max_live_count() < cmd->row_limit)) {
