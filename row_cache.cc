...
public:
    single_partition_populating_reader(schema_ptr s, row_cache& cache, mutation_source& underlying,
        mutation_reader delegate, const io_priority_class pc, query::clustering_key_filtering_context ck_filtering, tracing::trace_state_ptr trace_state)
        : _schema(std::move(s))
        , _cache(cache)
            ...
            return try_to_read(_cache._max_cached_partition_size_in_bytes, std::move(sm)).then(
                ...
                (is_wide_partition wide_partition, mutation_opt&& mo) {
                    if (wide_partition == is_wide_partition::no) {
                        ...
                        _large_partition_reader = _underlying(_schema, _large_partition_range, _ck_filtering, _pc, _trace_state);
...
class range_populating_reader final : public mutation_reader::impl {
    ...
    void update_reader() {
        if (_populate_phase != _cache._populate_phaser.phase()) {
            ...
            _reader = _underlying(_cache._schema, _range, query::no_clustering_key_filtering, _pc, _trace_state);
        }
    }
...
class scanning_and_populating_reader final : public mutation_reader::impl{
    ...
    class state_machine : public boost::static_visitor<future<streamed_mutation_opt>> {
        ...
        future<streamed_mutation_opt> switch_to_secondary_or_primary(just_cache_scanning_reader::cache_data&& data) {
            ...
            mutation_reader secondary = make_mutation_reader<range_populating_reader>(_cache, _schema, range, _ck_filtering, _pc, _trace_state,
    ...
    public:
        state_machine(schema_ptr s, row_cache& cache, const query::partition_range& range,
                      query::clustering_key_filtering_context ck_filtering, const io_priority_class& pc, tracing::trace_state_ptr trace_state)
            : _cache(cache)
            , _schema(std::move(s))
            , _range(range)
            , _pc(pc)
...
public:
    scanning_and_populating_reader(schema_ptr s,
                                    ...
                                    const io_priority_class& pc,
                                    tracing::trace_state_ptr trace_state)
        : _state_machine(s, cache, range, ck_filtering, pc, std::move(trace_state)) {}
    virtual future<streamed_mutation_opt> operator()() override {
...
row_cache::make_scanning_reader(schema_ptr s,
    ...
    return make_mutation_reader<scanning_and_populating_reader>(std::move(s), *this, range, ck_filtering, pc, std::move(trace_state));
}
...
row_cache::make_reader(schema_ptr s,
    ...
    if (range.is_singular()) {
        ...
        if (!pos.has_key()) {
            return make_scanning_reader(std::move(s), range, pc, ck_filtering, std::move(trace_state));
        }
        ...
        return _read_section(_tracker.region(), [&] {
          return with_linearized_managed_bytes([&] {
            ...
            if (i != _partitions.end() && i != _partitions.begin()) {
                ...
                if (e.wide_partition()) {
                    _tracker.on_uncached_wide_partition();
                    return _underlying(s, range, ck_filtering, pc, std::move(trace_state));
                }
                ...
                return make_mutation_reader<single_partition_populating_reader>(s, *this, _underlying,
                    _underlying(_schema, range, query::no_clustering_key_filtering, pc, trace_state), pc,
                    ck_filtering, trace_state);
            }
          });
        });
    }
