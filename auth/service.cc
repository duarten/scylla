...
future<permission_set> service::get_uncached_permissions(stdx::string_view role_name, const resource& r) const {
...
future<> create_role(
        ...
        const authentication_options& options) {
    return ser.underlying_role_manager().create(name, config).then([&ser, name, &config, &options] {
...
future<> alter_role(
    ...
    return ser.underlying_role_manager().alter(name, config_update).then([&ser, name, &options] {
        return ser.underlying_role_manager().can_login(name).then([&ser, name, &options](bool can_login) {
...
future<bool> has_role(const service& ser, stdx::string_view grantee, stdx::string_view name) {
    return when_all_succeed(
            ...
            validate_role_exists(ser, name)).then([name](role_set all_roles) {
