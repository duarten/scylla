...
future<bool> default_authorizer::any_granted() const {
    static const sstring query = sprint("SELECT * FROM %s.%s", meta::AUTH_KS, PERMISSIONS_CF);
    ...
    return _qp.process(
        ...
        return do_for_each(*results, [this](const cql3::untyped_result_set_row& row) {
            return do_with(
                    ...
                    permissions::from_strings(row.get_set<sstring>(PERMISSIONS_NAME)),
                    [this](const auto& username, const auto& r, const auto& perms) {
...
future<permission_set> default_authorizer::authorize(stdx::string_view role_name, const resource& r) const {
