...
SEASTAR_TEST_CASE(test_query_built_indexes_virtual_table) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        auto f1 = e.local_view_builder().wait_until_built("ks", "vcf", lowres_clock::now() + 2s);
        auto f2 = e.local_view_builder().wait_until_built("ks", idx, lowres_clock::now() + 2s);
        e.execute_cql("create materialized view vcf as select * from cf "
                      "where v is not null and p is not null "
                      "primary key (v, p)").get();
        e.execute_cql("create index idx on cf (v)").get();
        seastar::when_all_succeed(std::move(f1), std::move(f2)).get();
