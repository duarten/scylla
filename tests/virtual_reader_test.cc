...
SEASTAR_TEST_CASE(test_query_virtual_table) {
    return do_with_cql_env([] (auto& e) {
        ...
        auto ranges = db::size_estimates::size_estimates_mutation_reader::get_local_ranges();
        auto start_token1 = ranges[3].start;
        auto start_token2 = ranges[5].start;
        auto end_token1 = ranges[3].end;
        auto end_token2 = ranges[55].end;
        auto &qp = e.local_qp();
        return e.execute_cql("create table cf1(pk text PRIMARY KEY, v int);").discard_result().then([&e] {
            ...
            return qp.execute_internal(sprint("select * from system.size_estimates where keyspace_name = 'ks' "
                ...
                BOOST_REQUIRE_EQUAL(rs->size(), 506);
            });
