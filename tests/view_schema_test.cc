...
SEASTAR_TEST_CASE(test_base_non_pk_columns_in_view_partition_key_are_non_emtpy) {
    return do_with_cql_env_thread([] (cql_test_env& e) {
        ...
        for (auto&& view : views_matching) {
            ...
            auto f = e.local_view_builder().wait_until_built("ks", name, lowres_clock::now() + 5s).get();
        ...
        e.local_view_builder().wait_until_built("ks", name, lowres_clock::now() + 5s).get();
        auto msg = e.execute_cql(sprint("select p1, p2, c, v from %s", name)).get0();
        assert_that(msg).is_rows().is_empty();

        e.execute_cql("update cf set v = 'a' where p1 = 1 and p2 = ''").get();
        eventually([&] {
            auto msg = e.execute_cql(sprint("select p1, p2, c, v from %s", name)).get0();
            assert_that(msg).is_rows()
                    .with_size(1)
                    .with_row({
                            ...
                            {utf8_type->decompose(data_value(bytes()))},
                            {utf8_type->decompose(data_value(bytes()))},
                            {utf8_type->decompose(data_value(bytes("a")))},
                    });
        });

        e.execute_cql("update cf set v = '' where p1 = 1 and p2 = ''").get();
        eventually([&] {
            auto msg = e.execute_cql(sprint("select p1, p2, c, v from %s", name)).get0();
            assert_that(msg).is_rows().is_empty();
        });

        e.execute_cql("delete v from cf where p1 = 1 and p2 = ''").get();
        eventually([&] {
            auto msg = e.execute_cql(sprint("select p1, p2, c, v from %s", name)).get0();
            assert_that(msg).is_rows().is_empty();
        });
