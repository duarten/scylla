...
static range_tombstone rtie(int32_t start, int32_t end, api::timestamp_type timestamp) {
    return range_tombstone(key({start}), bound_kind::incl_start_bound, key({end}), bound_kind::excl_end_bound, {timestamp, gc_now});
}

static range_tombstone rtei(int32_t start, int32_t end, api::timestamp_type timestamp) {
    return range_tombstone(key({start}), bound_kind::excl_start_bound, key({end}), bound_kind::incl_end_bound, {timestamp, gc_now});
}

static range_tombstone at_least(int32_t start, api::timestamp_type timestamp) {
    return range_tombstone(bound(key({start}), bound_kind::incl_start_bound), bound::top(), {timestamp, gc_now});
}

static range_tombstone at_most(int32_t end, api::timestamp_type timestamp) {
    return range_tombstone(bound::bottom(), bound(key({end}), bound_kind::incl_end_bound), {timestamp, gc_now});
}

static range_tombstone less_than(int32_t end, api::timestamp_type timestamp) {
    return range_tombstone(bound::bottom(), bound(key({end}), bound_kind::excl_end_bound), {timestamp, gc_now});
}

static range_tombstone greater_than(int32_t start, api::timestamp_type timestamp) {
    return range_tombstone(bound(key({start}), bound_kind::excl_start_bound), bound::top(), {timestamp, gc_now});
}
...
BOOST_AUTO_TEST_CASE(test_search_prefixes) {
    range_tombstone_list l(*s);

    l.apply(*s, rt(0, 4, 5));

    BOOST_REQUIRE_EQUAL(5, l.search_tombstone_covering(*s, key({1, 2})).timestamp);
}
...
BOOST_AUTO_TEST_CASE(test_add_different_prefixes) {
    ...
    it = l.begin();
    assert_rt(range_tombstone(key({4}), bound_kind::incl_start_bound, key({4, 1}), bound_kind::excl_end_bound, {7, gc_now}), *it++);
    assert_rt(range_tombstone(key({4, 1}), bound_kind::incl_start_bound, key({4, 2}), bound_kind::incl_end_bound, {8, gc_now}), *it++);
    assert_rt(range_tombstone(key({4, 2}), bound_kind::excl_start_bound, key({4}), bound_kind::incl_end_bound, {7, gc_now}), *it++);
    BOOST_REQUIRE(it == l.end());
}
...
static std::vector<range_tombstone> make_random() {
    ...
    for (int32_t i = 0; i < size; ++i) {
        ...
        rts.emplace_back(key({next_start}),
                start_incl ? bound_kind::incl_start_bound : bound_kind::excl_start_bound,
                key({next_end}),
                end_incl ? bound_kind::incl_end_bound : bound_kind::excl_end_bound,
...
BOOST_AUTO_TEST_CASE(test_add_range_with_empty_end_to_range_with_empty_start_and_end) {
    ...
    l.apply(*s, range_tombstone(bound::bottom(), bound::top(), tombstone(2, gc_now)));
