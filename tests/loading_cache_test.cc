...
SEASTAR_TEST_CASE(test_loading_cache_loading_expiry_eviction) {
    return seastar::async([] {
        ...
        sleep(20ms).get();
        eventually([&] {
            if (loading_cache.find(0) != loading_cache.end()) {
                throw std::runtime_error("Expected the loading cache to be empty");
            }
        });
    });
}
...
SEASTAR_TEST_CASE(test_loading_cache_loading_reloading) {
    return seastar::async([] {
        ...
        eventually([&] {
            if (load_count < 2) {
                throw std::runtime_error(format("load_count is {}", load_count));
            }
        });
    });
}
