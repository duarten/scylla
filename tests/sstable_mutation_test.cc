        ...
        return do_with(sstp->read_rows(s), [sstp, s] (auto& reader) {
            return repeat([sstp, s, &reader] {
                return reader.read().then([s] (mutation_opt mut) {
                    ...
                    auto it = rts.begin();
                    BOOST_REQUIRE(it->start_bound().equal(*s, bound(make_ckey("aaa"), bound_kind::incl_start_bound)));
                    BOOST_REQUIRE(it->end_bound().equal(*s, bound(make_ckey("aaa", "bbb"), bound_kind::excl_end_bound)));
                    BOOST_REQUIRE(it->tomb.timestamp == 1459438519943668L);
                    ++it;
                    BOOST_REQUIRE(it->start_bound().equal(*s, bound(make_ckey("aaa", "bbb"), bound_kind::incl_start_bound)));
                    BOOST_REQUIRE(it->end_bound().equal(*s, bound(make_ckey("aaa", "bbb"), bound_kind::incl_end_bound)));
                    BOOST_REQUIRE(it->tomb.timestamp == 1459438519950348L);
                    ++it;
                    BOOST_REQUIRE(it->start_bound().equal(*s, bound(make_ckey("aaa", "bbb"), bound_kind::excl_start_bound)));
                    BOOST_REQUIRE(it->end_bound().equal(*s, bound(make_ckey("aaa"), bound_kind::incl_end_bound)));
                    BOOST_REQUIRE(it->tomb.timestamp == 1459438519943668L);
                    ++it;
