...
SEASTAR_TEST_CASE(test_async_loading_with_views) {
    return seastar::async([] {
        ...
        std::cout << "base " << s1_loaded->version() << std::endl;
        ...
        BOOST_REQUIRE_EQUAL(s1_loaded->registry_entry()->get_views().size(), 1);
        auto v1_from_s1 = s1_loaded->registry_entry()->get_views()[0];
        ...
        BOOST_REQUIRE_EQUAL(s2_loaded->registry_entry()->get_views().size(), 1);
        auto v2_from_s2 = s2_loaded->registry_entry()->get_views()[0];
