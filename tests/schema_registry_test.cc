...
SEASTAR_TEST_CASE(test_async_loading_with_views) {
    return seastar::async([] {
        ...
        auto sav1_loaded = local_schema_registry().get_or_load(s1->version(), [s1, v1] (table_schema_version) {
            return make_ready_future<frozen_schema_and_views>(frozen_schema_and_views(frozen_schema(s1), { frozen_schema(v1) }));
        }).get0();

        BOOST_REQUIRE(sav1_loaded.schema->version() == s1->version());
        auto s1_later = local_schema_registry().get_or_null(s1->version());
        BOOST_REQUIRE(s1_later);

        BOOST_REQUIRE_EQUAL(sav1_loaded.views.size(), 1);
        auto v1_from_s1 = sav1_loaded.views[0];
        ...
        auto sav2_loaded = local_schema_registry().get_or_load(s2->version(), [s2, v2] (table_schema_version) {
            return later().then([s2, v2] { return frozen_schema_and_views(frozen_schema(s2), { frozen_schema(v2) }); });
        }).get0();

        BOOST_REQUIRE(sav2_loaded.schema->version() == s2->version());
        auto s2_later = local_schema_registry().get_or_null(s2->version());
        BOOST_REQUIRE(s2_later);

        BOOST_REQUIRE_EQUAL(sav2_loaded.views.size(), 1);
        auto v2_from_s2 = sav2_loaded.views[0];
