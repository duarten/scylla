...
frozen_schema::frozen_schema(const schema_ptr& s)
    : _data([&s] {
        schema_mutations sm = (s->is_view() ? &db::schema_tables::make_view_mutations : &db::schema_tables::make_table_mutations)(s, api::new_timestamp(), true);
        bytes_ostream out;
...
schema_ptr frozen_schema::unfreeze() const {
    ...
    return (sv.mutations().is_view() ? &db::schema_tables::create_view_from_mutations : &db::schema_tables::create_table_from_mutations)(sv.mutations(), sv.version());
}
