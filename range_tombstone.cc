/*
 * Copyright (C) 2016 ScyllaDB
 */

/*
 * This file is part of Scylla.
 *
 * Scylla is free software: you can redistribute it and/or modify
 * it under the terms of the GNU Affero General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * Scylla is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with Scylla.  If not, see <http://www.gnu.org/licenses/>.
 */

#include "range_tombstone_list.hh"

std::ostream& operator<<(std::ostream& out, const bound_kind k) {
    switch(k) {
    case bound_kind::excl_end_bound:
        return out << "excl end";
    case bound_kind::incl_start_bound:
        return out << "incl start";
    case bound_kind::clustering:
        return out << "clustering";
    case bound_kind::incl_end_bound:
        return out << "incl end";
    case bound_kind::excl_start_bound:
        return out << "excl start";
    default:
        return out;
    }
}

bound_kind invert_kind(bound_kind k) {
    switch(k) {
    case bound_kind::excl_start_bound: return bound_kind::incl_end_bound;
    case bound_kind::incl_start_bound: return bound_kind::excl_end_bound;
    case bound_kind::excl_end_bound:   return bound_kind::incl_start_bound;
    case bound_kind::incl_end_bound:   return bound_kind::excl_start_bound;
    default: return k;
    }
}

const clustering_key_prefix bound::empty_prefix = clustering_key_prefix(std::vector<bytes>());

std::ostream& operator<<(std::ostream& out, const range_tombstone& rt) {
    if (rt) {
        return out << "{range_tombstone: start=" << rt.start_bound() << ", end=" << rt.end_bound() << ", " << rt.tomb << "}";
    } else {
        return out << "{range_tombstone: none}";
    }
}
