...
class range_tombstone_to_prefix_tombstone_converter {
    ...
    bound_kind _end_contiguous_bound;
public:
    ...
    std::experimental::optional<clustering_key_prefix> convert(const schema& s, const range_tombstone& rt) {
        if (!_open_tombstones.empty()) {
            ...
            // have a gap. If there is a gap while we're converting, it is
            // probably a bona-fide range delete, which we don't support.
            if (!_end_contiguous_delete.equal(s, rt.start)
                    || (_end_contiguous_bound == bound_kind::excl_end && rt.start_kind == bound_kind::excl_start)) {
                throw exceptions::unsupported_operation_exception(sprint(
        ...
        if (_open_tombstones.top().ends_with(s, rt.end)) {
            ret = _open_tombstones.top().release();
