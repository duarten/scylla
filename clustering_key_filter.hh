...
public:
    ...
    virtual const std::vector<range<clustering_key_prefix, can_wrap::no>>& get_ranges(const partition_key&) = 0;
...
public:
    ...
    const std::vector<range<clustering_key_prefix, can_wrap::no>>& get_ranges(const partition_key& key) const;
