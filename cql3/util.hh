...
template<typename T>
T do_with_parser(const sstring_view& cql, std::function<T (cql3_parser::CqlParser&)> f) {
...
static std::vector<relation_ptr> where_clause_to_relations(const sstring_view& where_clause) {
    return do_with_parser<std::vector<relation_ptr>>(where_clause, [] (cql3_parser::CqlParser& p) { return p.whereClause(); });
}
