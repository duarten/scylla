...
public:
    ...
    virtual future<streamed_mutation_opt> operator()() override {
        ...
        return get_local_ranges().then([&db, this] (auto&& ranges) {
            ...
            auto mutations = db::system_keyspace::make_size_estimates_mutation((*_current_partition)->name(), std::move(estimates));
            ++_current_partition;
...
private:
    ...
    estimates_for_current_keyspace(const database& db, std::vector<token_range> local_ranges) const {
        ...
        auto cf_names = boost::copy_range<std::vector<bytes>>(keyspace->cf_meta_data() | boost::adaptors::transformed([] (auto&& cf) {
        ...
        for (auto& range : _slice.get_all_ranges()) {
            ...
            for (auto&& r : rows_to_estimate) {
                auto& cf = db.find_column_family(keyspace->name(), utf8_type->to_string(r.cf_name));
    ...
    static ks_range get_keyspaces(const schema& s, const database& db, query::partition_range range) {
        struct keyspace_comparator {
            ...
            keyspace_comparator(const schema& s) : _s(s) , _cmp(_s) { }
            dht::ring_position as_ring_position(const keyspace_metadata& ks) {
                auto pkey = partition_key::from_single_value(_s, utf8_type->decompose(ks.name()));
                return dht::global_partitioner().decorate_key(_s, std::move(pkey));
            }
        };
        struct keyspace_less_comparator : public keyspace_comparator {
            keyspace_less_comparator(const schema& s) : keyspace_comparator(s) { }
        ...
        struct keyspace_tri_comparator : public keyspace_comparator {
            keyspace_tri_comparator(const schema& s) : keyspace_comparator(s) { }
        ...
        boost::sort(keyspaces, keyspace_less_comparator(s));
        return boost::copy_range<ks_range>(range.slice(std::move(keyspaces), keyspace_tri_comparator(s)));
    }
