...
private:
    ...
    static ks_range get_keyspaces(const schema& s, const database& db, query::partition_range range) {
        struct keyspace_less_comparator {
            ...
            bool operator()(lw_shared_ptr<keyspace_metadata> ks, const dht::ring_position& rp) {
                return as_ring_position(*ks).less_compare(_s, rp);
            }
            bool operator()(const dht::ring_position& rp, lw_shared_ptr<keyspace_metadata> ks) {
        ...
        return boost::copy_range<ks_range>(range.slice(keyspaces, keyspace_less_comparator(s)));
    }
