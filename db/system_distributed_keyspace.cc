...
future<> system_distributed_keyspace::start() {
    ...
    // See issue #2129.
    return _mm.announce_new_keyspace(ksm, api::min_timestamp, false).then([this] {
...
future<std::unordered_map<utils::UUID, sstring>> system_distributed_keyspace::view_status(sstring ks_name, sstring view_name) const {
    return _qp.process(
        ...
        std::unordered_map<utils::UUID, sstring> view_status;
        for (auto& row : *cql_result) {
            auto host_id = row.get_as<utils::UUID>("host_id");
            auto status = row.get_as<sstring>("status");
            view_status.emplace(std::move(host_id), status);
        }
        return view_status;
    });
}
