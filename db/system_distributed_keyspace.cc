...
future<> system_distributed_keyspace::start() {
    ...
    auto ignore_existing = [] (seastar::noncopyable_function<future<>()> func) {
    ...
    }).then([this, ignore_existing] {
        return do_with(all_tables(), [this, ignore_existing = std::move(ignore_existing)] (std::vector<schema_ptr>& tables) {
            return do_for_each(tables, [this, ignore_existing = std::move(ignore_existing)] (schema_ptr table) {
...
future<> system_distributed_keyspace::view_built(sstring ks_name, sstring view_name) const {
    return db::system_keyspace::get_local_host_id().then([this, ks_name = std::move(ks_name), view_name = std::move(view_name)] (utils::UUID host_id) {
        return _qp.process(
