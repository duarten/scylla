    ...
    std::map<qualified_name, schema_mutations>&& views_after)
{
    ...
    proxy.local().get_db().invoke_on_all([&] (database& db) {
        return seastar::async([&] {
            ...
            for (auto&& dt : boost::range::join(tables_diff.dropped, views_diff.dropped)) {
                db.drop_column_family(dt.schema.get()->ks_name(), dt.schema.get()->cf_name(), [&] { return dt.jp.value(); }).get();
            }
            ...
            for (auto f = notifications.rbegin(); f != notifications.rend(); ++f) {
                f->get();
            }
        });
    }).get();
