    ...
    std::map<qualified_name, schema_mutations>&& after)
{
    return merge_schemas(proxy,
            ...
            std::mem_fn(&service::migration_manager::notify_create_view),
            std::mem_fn(&service::migration_manager::notify_update_view),
            std::mem_fn(&service::migration_manager::notify_drop_view));
}
...
std::vector<mutation> make_create_keyspace_mutations(lw_shared_ptr<keyspace_metadata> keyspace, api::timestamp_type timestamp, bool with_tables_and_types_and_functions)
{
    ...
    if (with_tables_and_types_and_functions) {
        ...
        for (auto&& s : keyspace->tables()) {
            add_table_to_schema_mutation(s, timestamp, true, mutations);
        }
        for (auto&& s : keyspace->views()) {
            assert(s->view_info());
            add_view_to_schema_mutation(s, timestamp, true, mutations);
        }
    }
    return mutations;
}
...
schema_ptr create_view_from_mutations(schema_mutations sm, std::experimental::optional<table_schema_version> version)  {
    auto table_rs = query::result_set(sm.columnfamilies_mutation());
...
static future<schema_ptr> create_view_from_table_row(distributed<service::storage_proxy>& proxy, const query::result_set_row& row) {
 ...
 */
future<std::vector<schema_ptr>> create_views_from_schema_partition(distributed<service::storage_proxy>& proxy, const schema_result::mapped_type& result)
{
    auto views = make_lw_shared<std::vector<schema_ptr>>();
    return parallel_for_each(result->rows().begin(), result->rows().end(), [&proxy, views = std::move(views)] (auto&& row) {
        return create_view_from_table_row(proxy, row).then([views] (schema_ptr&& v) {
...
schema_mutations make_view_mutations(schema_ptr view, api::timestamp_type timestamp, bool with_columns)
{
    if (!view->is_view()) {
        throw exceptions::invalid_request_exception(sprint("Table %s.%s is not a Materialized Viewd", view->ks_name(), view->cf_name()));
    }
    // When adding new schema properties, don't set cells for default values so that
    // both old and new nodes will see the same version during rolling upgrades.
...
void add_view_to_schema_mutation(schema_ptr view, api::timestamp_type timestamp, bool with_columns, std::vector<mutation>& mutations)
{
...
std::vector<mutation> make_drop_view_mutations(lw_shared_ptr<keyspace_metadata> keyspace, schema_ptr view, api::timestamp_type timestamp) {
    if (!view->is_view()) {
        throw exceptions::invalid_request_exception(sprint("Table %s.%s is not a Materilized View", view->ks_name(), view->cf_name()));
    }
    // Include the serialized keyspace in case the target node missed a CREATE KEYSPACE migration (see CASSANDRA-5631).
    auto mutations = make_create_keyspace_mutations(keyspace, timestamp, false);
