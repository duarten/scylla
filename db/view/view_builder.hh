 ...
 *     we ensure that if the view is in the in-progress system table, then it may
 *     not have been written to this table. We don't load the built views from this table
 *     when starting. When starting, the following happens:
 *          * If the view is in the system.built_views table and not the in-progress
 *            system table, then it will be in this one;
 *          * If the view is in the system.built_views table and not in this one, it will
 *            still be in the in-progress system table - we detect this and mark it as built
 *            in this table too, keeping the invariant;
...
class view_builder final : public service::migration_listener::only_view_notifications {
     ...
     * view. Adding a view v' overwrites the set of pinned sstables, regardless of there
     * being another view v'' being built. The new set will potentially contain new data
     * already in v'', written as part of the write path. We assume this case is rare and
     * optimize for fewer disk space in detriment of network bandwidth. The sstables are
     * pinned by the flat_mutation_reader.
     */
    ...
    seastar::sharded<view_builder>& _buddies;
    database& _db;
    ...
    // Uses to coordinate between shards the conclusion of the build process for a particular view.
...
public:
    view_builder(seastar::sharded<view_builder>&, database&, db::system_distributed_keyspace&, service::migration_manager&);
