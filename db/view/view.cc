        ...
        std::vector<mutation> mutations)
{
    ...
    for (auto& mut : mutations) {
        ...
        if (paired_endpoint) {
            ...
            fs->push_back(service::get_local_storage_proxy().send_to_endpoint(std::move(mut), target, std::move(pending_endpoints), db::write_type::VIEW).handle_exception([target] (auto ep) {
