...
#endif
    return seastar::when_all_succeed(fs->begin(), fs->end()).finally([fs = std::move(fs)] { });
}

view_builder::view_builder(seastar::sharded<view_builder>& buddies, database& db, db::system_distributed_keyspace& sys_dist_ks, service::migration_manager& mm)
        : _buddies(buddies)
        , _db(db)
...
void view_builder::load_view_status(view_builder::view_build_status status, std::unordered_set<utils::UUID>& loaded_views) {
    ...
    vlogger.info0("Resuming to build view {}.{}", status.view->ks_name(), status.view->cf_name());
...
future<> view_builder::calculate_shard_build_step(
    ...
    for (auto& [view_name, first_token, next_token_opt, cpu_id] : in_progress) {
        if (auto view = maybe_fetch_view(view_name)) {
            if (built_views.find(view->id()) != built_views.end()) {
                if (engine().cpu_id() == 0) {
                    auto f = _sys_dist_ks.view_built(std::move(view_name.first), std::move(view_name.second)).then([view = std::move(view)] {
    ...
    auto is_new = [&] (const view_ptr& v) {
        return loaded_views.find(v->id()) == loaded_views.end() && built_views.find(v->id()) == built_views.end();
    };
...
future<> view_builder::add_new_view(view_ptr view, build_step& step) {
    vlogger.info0("Building view {}.{}", view->ks_name(), view->cf_name());
...
class view_builder::consumer {
    ...
    build_step& _step;
    std::vector<view_build_status> _built_views;
...
public:
    ...
    void check_for_built_views() {
        for (auto it = _step.build_status.begin(); it != _step.build_status.end();) {
            ...
            // shard-owned token. We finish building the view when the next_token to build is at, or
            // just before the next token, but the shard-owned current token is after or at the first token.
            ...
            if (it->next_token && *it->next_token <= it->first_token && _step.current_token() >= it->first_token) {
                _built_views.push_back(std::move(*it));
    ...
    std::vector<view_build_status> consume_end_of_stream() {
        ...
        vlogger.info("Completed build step for base {}.{}, at token {}", _step.base->schema()->ks_name(), _step.base->schema()->cf_name(), _step.current_token());
...
void view_builder::execute(build_step& step, exponential_backoff_retry r) {
    ...
    for (auto& [view, first_token, _] : built) {
...
future<> view_builder::maybe_mark_view_as_built(view_ptr view, dht::token next_token) {
    ...
    return _buddies.map_reduce0(
            [view_id = view->id()] (view_builder& builder) {
                return builder._built_views.find(view_id) != builder._built_views.end();
            },
            true,
        ...
        if (built) {
            ...
            return seastar::when_all_succeed(
                    ...
                    _sys_dist_ks.view_built(view->ks_name(), view->cf_name())).then([view] {
                system_keyspace::remove_view_build_progress_across_all_shards(view->cf_name(), view->ks_name());
            }).finally([this, view_id = view->id()] {
                return _buddies.invoke_on_all([view_id] (view_builder& builder) {
                    builder._built_views.erase(view_id);
                    if (engine().cpu_id() == 0) {
                        auto view = builder._db.find_schema(view_id);
                        auto it = builder._build_notifiers.find(std::make_pair(view->ks_name(), view->cf_name()));
...
future<> view_builder::wait_until_built(const sstring& ks_name, const sstring& view_name, lowres_clock::time_point timeout) {
    return _buddies.invoke_on(0, [ks_name, view_name, timeout] (view_builder& builder) {
        auto v = std::make_pair(std::move(ks_name), std::move(view_name));
