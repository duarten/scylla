 ...
 * view replicas. The future resolves when the updates have been generated, i.e., propagating the view
 * updates to the view replicas happens synchronously.
 *
 * @param base the base schema at a particular version.
 * @param views the affected views which need to be updated.
 * @param base_token The token to use to match the base replica with the paired replicas.
 * @param reader the base table updates being applied.
...
future<> column_family::populate_views(
    ...
    return db::view::generate_view_updates(
        ...
        int i = 0;
        for (auto&& m : updates) {
            i += m.partition().clustered_rows().calculate_size();
        }
