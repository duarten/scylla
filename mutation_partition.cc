...
mutation_partition::range_tombstone_for_row(const schema& schema, const clustering_key& key) const {
    ...
    for (unsigned prefix_len = 1; prefix_len <= key_length; ++prefix_len) {
        auto found_t = _row_tombstones.search_tombstone_covering(schema, key.prefix_view(schema, prefix_len));
        if (found_t) {
            t.apply(found_t);
        }
    }
...
mutation_partition::apply_row_tombstone(const schema& schema, clustering_key_prefix prefix, tombstone t) {
    ...
    auto start = prefix;
    _row_tombstones.add(schema, start, std::move(prefix), std::move(t));
}

void
mutation_partition::apply_row_tombstone(const schema& schema, const range_tombstone& rt) {
    _row_tombstones.add(schema, rt);
}
...
mutation_partition mutation_partition::difference(schema_ptr s, const mutation_partition& other) const
{
    ...
    for (auto&& rt : _row_tombstones) {
        while (it_rt != other._row_tombstones.end() && cmp_rt(it_rt->start, rt.start)) {
            ++it_rt;
        }
        if (it_rt == other._row_tombstones.end()
                || !it_rt->start.equal(*s, rt.start)
                || !it_rt->stop.equal(*s, rt.stop)
                || rt.tomb > it_rt->tomb) {
