...
static void write_tombstones(const schema& s, auto& row_tombstones, const range_tombstone_list& rt_list)
{
    if (service::get_local_storage_service().cluster_supports_range_tombstones()) {
        for (auto&& rt : rt_list) {
            row_tombstones.add().write_start(rt.start).write_tomb(rt.tomb).write_start_kind(bound_kind::incl_start_bound)
                .write_end(rt.end).write_end_kind(bound_kind::incl_end_bound).end_range_tombstone();
        }
    } else {
        range_tombstone_merger m;
        for (auto&& rt : rt_list) {
            auto prefix = m.merge(s, rt);
            if (prefix) {
                row_tombstones.add().write_start(*prefix).write_tomb(rt.tomb).write_start_kind(bound_kind::incl_start_bound)
                    .write_end(*prefix).write_end_kind(bound_kind::incl_end_bound).end_range_tombstone();
            }
        }
    }
}
