...
struct bound_kind_weight_compare {
    // Bound kinds are ordered by their weight; see bound::compare.
    int32_t to_weight(bound_kind k) const {
        switch(k) {
    ...
    bool operator()(bound_kind k1, bound_kind k2) const {
        return to_weight(k1) < to_weight(k2);
    }
};
...
public:
    ...
    struct compare {
        ...
        bool operator()(const clustering_key_prefix& p1, bound_kind k1, const clustering_key_prefix& p2, bound_kind k2) const {
            ...
            if (d1 == d2) {
                return bound_kind_weight_compare()(k1, k2);
            }
